...
def run(args, run_id):
    ...
    for t,ncla in args.taskcla:
        ...
        train_loss = trval_stats[0]
        val_loss = trval_stats[1]
        with open(os.path.join(args.checkpoint, args.stat_log), 'a') as stat_log:
            stat_log.write('{}, {:.4f}, {:.4f}\n'.format(t, train_loss, val_loss))
...
def main(args):
    ...
    for n in range(args.num_runs):
        ...
        args.stat_log = '{}_{}_tasks_seed_{}.csv'.format(args.experiment, args.ntasks, args.seed)
        print("args.checkpoint: {}".format(args.checkpoint))
        print("args.output: {}".format(args.output))

        with open(os.path.join(args.checkpoint, args.stat_log), 'w') as stat_log:
            stat_log.write('task_id, train_loss, val_loss\n')
