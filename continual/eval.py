...
def eval_tasks(model, tasks, args):
    ...
    for i, task in enumerate(tasks):
        ...
        for b_from in range(0, x.size(0), eval_bs):
            ...
            with torch.no_grad():
                xb = Variable(xb)
            features = model(xb, t).data.cpu()
            all_features[t].append(features.numpy())
            all_y[t].append(yb.cpu().numpy())
            _, pb = torch.max(features, 1, keepdim=False)
            rt += (pb == yb).float().sum()
    ...
    return result, [all_features,all_y]
...
def life_experience_stat(model, continuum, x_te, args, stat_file):
    ...
    result, features_y = eval_tasks(model, x_te, args)
    ...
    with open(os.path.join(args.save_path, args.feat_file), 'wb') as fp:
        pickle.dump(features_y, fp)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', type=str, default='results/analysis/',
    ...
    if args.pretrained != '':
        ...
        model.load_state_dict(torch.load(args.pretrained)[2])
    ...
    args.feat_file = args.model + '_' + args.data_file
